...
impl CorpusConfigLoad {
    fn check_for_language(language: &str) -> Result<PathBuf, String> {
        let try_find_path = glob("static/corpus_configs/*/*.toml").unwrap().flatten().find(|stem| {
            return language == stem.file_stem().unwrap_or_else(|| std::ffi::OsStr::new(""));
        });

        return match try_find_path {
            | Some(path) => {
                let res = path.as_path().parent().unwrap().components().last().unwrap().as_os_str();
    ...
    pub fn new(language: &str, preferred_folder: Option<&str>) -> Result<Self, String> {
        ...
        return match preferred_folder {
            | Ok(preferred_folder) => {
                ...
                let path = PathBuf::from("../../static").join("corpus_configs").join(preferred_folder).join(file_name.clone());
                ...
                f.read_to_string(&mut buf).map_err(|_| "Toml contains non-utf8 characters, aborting...".to_string())?;
...
impl CorpusConfig {
    ...
    pub fn all() -> Vec<(String, Self)> {
        return glob("static/text/*").unwrap().flatten().filter(|pb| {
            return pb.is_dir();
        }).flat_map(|pb| {
    ...
    pub fn translator(self) -> Translator {
        let mut res = Translator::new().letters_to_lowercase(&self.letters_to_lowercase).keep(&self.keep).one_to_one(&self.one_to_one.1, &self.one_to_one.0).custom_unshift(&self.punct_unshifted.1, &self.punct_unshifted.0).as_multiple_string(&self.to_multiple).build();
