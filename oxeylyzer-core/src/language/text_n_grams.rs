...
pub struct TextNgrams<'a, const N: usize> {
    pub n_grams: HashMap<&'a str, usize>,
}
...
impl<'a, const N: usize> TextNgrams<'a, N> {
            ...
            M: SmartStringMode,
    {
        ...
        it1.zip(it2).map(|(i1, i2)| &last[i1..i2]).for_each(|ngram| {
            ngrams.entry(ngram).and_modify(|f| *f += 1).or_insert(1);
        });

        return Self { n_grams: ngrams };
    }

    pub fn combine_with(mut self, rhs: Self) -> Self {
        for (trigram, freq) in rhs.n_grams.into_iter() {
            self.n_grams.entry(trigram).and_modify(|f| *f += freq).or_insert(freq);
        }
