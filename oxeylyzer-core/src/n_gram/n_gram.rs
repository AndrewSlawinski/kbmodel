...
impl NGram<Finger, 2> {
    pub fn get_bigram_pattern(&self) -> BigramType {
        return if self.is_same_finger_n_gram(2)
        {
            SameFingerBigram
        } else {
            BigramType::Other
...
impl NGram<Finger, 3> {
    pub fn get_trigram_pattern(&self) -> TrigramType {
        return if self.is_same_finger_n_gram(3)
        {
            SameFingerTrigram
...
impl<const N: usize> NGram<Finger, N> {
    ...
    fn is_redirect(&self) -> bool {
        ...
        return self.is_one_hand() && (self.inner[0].lt(self.inner[1]) == self.inner[1].gt(self.inner[2]));
    }
    ...
    fn is_same_finger_n_gram(&self, n: u8) -> bool {
        ...
        for i in (n - 1) as usize..self.inner.len() {
            if self.inner[(i - 1)..i].iter().all(|x| *x == self.inner[i - 1])
            {
                return true;
            }
        }
...
impl<const N: usize> From<&[&NGram<char, 1>; N]> for NGram<char, N> {
    fn from(value: &[&NGram<char, 1>; N]) -> Self {
        ...
        value.iter().enumerate().for_each(|(i, t)| v[i] = t.inner.to_vec()[0]);
